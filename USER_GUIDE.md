# Plex Play 3.19.21 사용 설명서

Plex Play는 개인 Plex Media Server의 콘텐츠를 Android 휴대폰, 태블릿·폴더블,
Android TV·OTT 기기에서 탐색하고 재생하는 앱입니다. 이 문서는 3.19.21 버전을
기준으로 작성되었습니다.
...
재생 준비 단계에서는 이전에 선택된 서버 자막을 먼저 해제합니다. 호환 변환 재생에서
`자동` 또는 내장 자막을 선택하면 앱이 해당 Plex 자막 스트림을 새 변환 세션에 지정하고
현재 위치에서 이어서 재생합니다. `끔`을 선택하면 자막 없는 새 변환 세션으로 바뀝니다.
...
- 호환 변환의 한국어 내장 자막은 `자동`에서 우선 선택되며, 다른 내장 자막이나 `끔`을
  선택하면 즉시 새 변환 세션에 반영됩니다.
