# Plex Play 3.19.22 사용 설명서

Plex Play는 개인 Plex Media Server의 콘텐츠를 Android 휴대폰, 태블릿·폴더블,
Android TV·OTT 기기에서 탐색하고 재생하는 앱입니다. 이 문서는 3.19.22 버전을
기준으로 작성되었습니다.
...
- 상세 화면의 초록 표시는 원본 재생 지원, 금색 표시는 Plex 호환 변환이 자동으로
  적용되는 항목을 뜻합니다.
...
원본 화질은 기기의 비디오·오디오 디코더와 HDR 출력이 해당 형식을 지원할 때 Plex
원본 스트림을 직접 재생합니다. AV1, Dolby Vision, HDR10+, HDR10, HLG 또는 Dolby
오디오를 기기가 직접 처리할 수 없으면 Plex 서버가 1080p 이하 H.264/AAC HLS로
호환 변환합니다. 1080p, 720p, 480p를 직접 선택한 경우에도 H.264/AAC 호환 변환을
사용합니다. 서버에서 하드웨어/소프트웨어 트랜스코딩을 사용할 수 있어야 합니다.
재생 준비 단계에서는 이전에 선택된 서버 자막을 먼저 해제하고 호환 영상을 자막 없이
안정적으로 재생합니다. 재생 설정에서 `자동` 또는 내장 자막을 선택하면 앱이 해당 Plex
자막 스트림을 새 변환 세션에 지정하고 현재 위치에서 이어서 재생합니다. 자막 변환에
실패하면 자동으로 자막 없는 호환 재생으로 복구합니다.
...
- 호환 변환 중에도 Plex 내장 자막의 언어·이름·코덱 목록을 표시합니다. 내장 자막을
  선택하면 해당 스트림으로 Plex 변환 세션을 다시 만들고 현재 위치에서 이어 봅니다.
- 호환 변환에서 `자동`을 선택하면 한국어 내장 자막을 우선 사용합니다. 다른 내장 자막이나
  `끔`을 선택하면 현재 위치를 유지한 새 변환 세션에 반영됩니다.
- 텍스트형 Plex 자막은 앱에서 직접 표시하며 사용자 글꼴·크기·색상·외곽선·위치
  설정을 사용할 수 있습니다.
- 자동 자막의 우선 언어는 한국어
- 텍스트 자막 글꼴 즉시 적용
...
사용자 자막 화면을 다시 그려 현재 재생에 반영하고 다음 재생에도 유지됩니다.
...
- 호환 변환 영상에서 자막을 바꾸면 새 변환 세션 준비를 위해 잠시 기다립니다.
